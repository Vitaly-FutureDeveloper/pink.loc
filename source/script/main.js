...
(function(){
//*************************************************
//****Изменение цветов формы страницы photo**** >>
//*************************************************
	var itemsColorOptions = document.querySelector('.photo-form__icon-list');
	var itemsColorOption = document.querySelectorAll('.photo-form__icon-item'),
		itemsColorPic = document.querySelectorAll('.photo-form__icon-pic');

	function colorChanger(evt){
		evt.preventDefault();
		//evt.stopPropagation();
		
		itemsColorPic.forEach( (item, i) => {
			item.classList.remove('photo-form__icon-pic--active');
			console.log(evt.target);
			
			if(evt.target == itemsColorPic[i]){
				item.classList.add('photo-form__icon-pic--active');
				console.log(evt.target);
			}
		});
	}

	itemsColorOptions.addEventListener('click', colorChanger);
